---
permalink: /zh/
title: "关于我"
excerpt: "关于我"
author_profile: true
redirect_from: 
  - /zh/about/
  - /zh/about.html
---

[English Version](/) | **中文版**

## 个人简介

我是吴左晗，目前在[香港科技大学（广州）](https://www.hkust-gz.edu.cn/)[数据科学与分析学域](https://www.hkust-gz.edu.cn/academics/hubs-and-thrust-areas/information-hub/data-science-and-analytics/)攻读博士学位，有幸师从[陈雷教授](https://www.hkust-gz.edu.cn/people/lei-chen/)（IEEE Fellow, ACM Fellow）和张永琦教授，在[大数据研究所](https://www.hkust-gz.edu.cn/research/)开展研究工作。

2024年，我以优异成绩（GPA: 3.9/4.0，排名: 7/71，前10%）获得中山大学人工智能学院工学学士学位，本科毕业论文获评校级优秀论文（指导老师：郑立彬教授）。

## 研究兴趣

我的研究聚焦于开发智能系统，将数据工程与分析中复杂且耗费人力的任务自动化，从而让数据驱动的洞见能够被更广泛地获取和应用。目前的研究兴趣包括：

- **大语言模型（LLMs）**在数据工程和分析中的应用
- **数据挖掘**与知识发现
- **深度学习**在实际数据系统中的应用
- **强化学习**用于动态优化问题

## 最新动态

- **[2025年10月]** 我们关于零样本日志异常检测的论文被 **ICDE 2026** 接收！
- **[2025年6月]** 我们关于基于强化学习的定价策略的论文在 **IEEE TKDE** 发表！
- **[2024年9月]** 开启在港科大（广州）的博士研究之旅。
- **[2023年4月]** 在美国加州阿纳海姆举行的 **ICDE 2023** 上展示了我们关于枢纽型出行服务的工作。

## 联系方式

- **邮箱**: zh.wu [at] connect.hkust-gz.edu.cn
- **GitHub**: [hehepig4](https://github.com/hehepig4)
- **Google Scholar**: [个人主页](https://scholar.google.com/citations?user=mf2TLIIAAAAJ)
- **ORCID**: [0000-0001-6166-3690](https://orcid.org/0000-0001-6166-3690)

---

*"自动化复杂性，普及洞见力。"*
